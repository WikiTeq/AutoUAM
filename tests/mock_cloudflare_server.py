...
class MockCloudflareServer:
    ...
    async def set_security_level(self, request: web.Request) -> web.Response:
        ...
        if self.rate_limit_counter >= 10:
            return web.json_response(
                {"success": False, "errors": [{"code": 1015, "message": "Rate limit exceeded"}]},
                status=429,
        ...
        try:
            ...
            return web.json_response(
                {"success": False, "errors": [{"code": 6001, "message": "Invalid JSON"}]},
                status=400
            )
        ...
        if new_level not in valid_levels:
            return web.json_response(
                {"success": False, "errors": [{"code": 1004, "message": "Invalid security level"}]},
                status=400
            )
