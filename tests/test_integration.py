...
class TestUAMManagerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_uam_manager_enable_uam(self, mock_settings, mock_cloudflare_response):
        """Test enabling UAM."""
        with patch('autouam.core.uam_manager.CloudflareClient') as mock_client_class:
            # Mock the client instance and its async context manager
            mock_client_instance = MagicMock()
            ...
            mock_client_instance.__aexit__ = AsyncMock(return_value=None)
            mock_client_instance.test_connection = AsyncMock(return_value=True)
            mock_client_instance.enable_under_attack_mode = AsyncMock(return_value=mock_cloudflare_response)
            mock_client_class.return_value = mock_client_instance
    ...
    @pytest.mark.asyncio
    async def test_uam_manager_disable_uam(self, mock_settings, mock_cloudflare_response):
        """Test disabling UAM."""
        with patch('autouam.core.uam_manager.CloudflareClient') as mock_client_class:
            # Mock the client instance and its async context manager
            mock_client_instance = MagicMock()
            ...
            mock_client_instance.__aexit__ = AsyncMock(return_value=None)
            mock_client_instance.test_connection = AsyncMock(return_value=True)
            mock_client_instance.disable_under_attack_mode = AsyncMock(return_value=mock_cloudflare_response)
            mock_client_class.return_value = mock_client_instance
            ...
            assert result is True
            mock_client_instance.disable_under_attack_mode.assert_called_once_with(regular_mode="essentially_off")
    ...
    async def test_uam_manager_check_once(self, mock_settings):
        ...
        with patch('autouam.core.uam_manager.CloudflareClient') as mock_client_class:
            ...
            # Mock high load
            with patch('autouam.core.monitor.LoadMonitor.get_normalized_load', return_value=30.0):
                manager = UAMManager(mock_settings)
    ...
    async def test_uam_manager_api_error_handling(self, mock_settings):
        ...
        with patch('autouam.core.uam_manager.CloudflareClient') as mock_client_class:
            ...
            mock_client_instance.__aenter__ = AsyncMock(return_value=mock_client_instance)
            mock_client_instance.__aexit__ = AsyncMock(return_value=None)
            mock_client_instance.test_connection = AsyncMock(side_effect=ClientResponseError(
    ...
    async def test_uam_manager_rate_limiting(self, mock_settings):
        ...
        with patch('autouam.core.uam_manager.CloudflareClient') as mock_client_class:
            ...
            mock_client_instance.__aexit__ = AsyncMock(return_value=None)
            mock_client_instance.test_connection = AsyncMock(return_value=True)
            mock_client_instance.enable_under_attack_mode = AsyncMock(side_effect=[
                ClientResponseError(request_info=MagicMock(), history=[], status=429, message="Rate limited"),
            ...
            # This will fail because the CloudflareClient's retry logic is not being tested here
            # The rate limiting happens inside the CloudflareClient, not at the UAMManager level
            result = await manager.enable_uam_manual()
...
class TestHealthCheckerIntegration:
    ...
    async def test_health_checker_all_healthy(self, mock_settings):
        ...
        with patch('autouam.health.checks.CloudflareClient') as mock_client_class:
            ...
            mock_client_instance.__aexit__ = AsyncMock(return_value=None)
            mock_client_instance.test_connection = AsyncMock(return_value=True)
            mock_client_instance.get_current_security_level = AsyncMock(return_value="essentially_off")
            mock_client_class.return_value = mock_client_instance
